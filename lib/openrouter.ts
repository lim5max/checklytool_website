  ...
  "essay_analysis": {
    ...
    "errors": {
      ...
      "examples": [
        ...
        "орфографическая - вторая ошибка описание",
...
- Найди ТОЛЬКО реальные ошибки, классифицируй их по типам:
  ...
  * ВАЖНО: неполные предложения (без сказуемого или подлежащего) - это СИНТАКСИЧЕСКАЯ ошибка
...
export async function analyzeEssayTwoStage(
	...
	try {
		...
		const proSystemPrompt = `Ты - OCR эксперт. Твоя задача - прочитать МАКСИМАЛЬНО ТОЧНО рукописный текст со сканированного изображения.
...
1. Читай буква за буквой, слово за словом
2. Если не уверена в букве/слове - выбери САМЫЙ вероятный вариант по контексту
3. СОХРАНЯЙ пунктуацию и структуру оригинала (абзацы, отступы)
4. Если текст нечитаемый в строке - напиши [неразборчиво]
5. Не добавляй слова, которых нет в оригинале
6. Не исправляй орфографические и грамматические ошибки - сохраняй как есть
7. Разделяй слипшиеся слова, если это артефакт почерка (не намеренное слитное написание)

КРИТИЧЕСКИ ВАЖНО:
- Исправляй ТОЛЬКО неразборчивый почерк (используя контекст)
- НЕ исправляй реальные ошибки ученика (орфография, грамматика, пунктуация)
- Если сомневаешься - оставь как написано или используй [неразборчиво]
...
- Сохраняй структуру абзацев через \n`
		...
		const proRequest: OpenRouterRequest = {
			...
			max_tokens: 4000,
			temperature: 0.3, // Оптимальная температура для гибкого распознавания контекста
			top_p: 0.85, // Баланс между точностью и контекстной вариативностью
			top_k: 40, // Ограничение выбора токенов для стабильности
			metadata: {
		...
		// Постобработка текста: исправление артефактов OCR
		const extractedText = proResult.raw_text
			// Убираем странные слипшиеся слова с одиночными буквами
			.replace(/([а-яa-z])([А-ЯA-Z])\s{1}([а-яa-z])/g, '$1$2$3')
			// Убираем множественные пробелы
			.replace(/\s{2,}/g, ' ')
			// Правильные пробелы после пунктуации
			.replace(/([.!?])\s{1}([а-яА-ЯёЁa-zA-Z])/g, '$1 $2')
		...
		const flashSystemPrompt = `Ты - ОПЫТНЫЙ преподаватель русского/английского языка для проверки сочинений 9-го класса.

Тебе предоставлен расшифрованный текст сочинения ученика.
⚠️ ВАЖНО: Текст был распознан из рукописи через OCR. Возможны небольшие артефакты распознавания.

Твоя задача - ПРОВЕРИТЬ текст на РЕАЛЬНЫЕ ошибки и ОЦЕНИТЬ по критериям.
...
ПРОВЕРЯЙ ТОЛЬКО ДЕЙСТВИТЕЛЬНЫЕ ОШИБКИ:
1. Явные орфографические ошибки (не артефакты OCR)
2. Пропущенные запятые (перед "который", "что", союзами "и", "но", "или")
3. Грамматическое согласование (неправильные окончания, формы слов)
4. Пунктуация (кроме явных артефактов распознавания)
5. Речевые и синтаксические ошибки

⚠️ ВАЖНО для пунктуационных ошибок:
- НЕ придумывай ошибки про отсутствующие запятые, если они ЕСТЬ в тексте
- Перед тем как указать "отсутствует запятая перед X" - проверь, что её ДЕЙСТВИТЕЛЬНО нет
- Если запятая есть, но в неправильном месте - опиши точно где проблема

ИГНОРИРУЙ:
- Одиночные странные символы (артефакты OCR)
- Невозможные слова (< 1 символа или > 30 символов)
- Расколотые слова (артефакты распознавания)
- Фрагментированные предложения (могут быть артефактами OCR)

Требования к анализу:
- Оцени структуру: есть ли вступление, основная часть, заключение
- Оцени логику изложения и связность текста
- Найди ТОЛЬКО реальные ошибки, классифицируй их по типам
- КРИТИЧЕСКИ ВАЖНО: В массив examples добавь ВСЕ найденные ошибки (не только примеры!)
- Количество элементов в examples должно равняться total_errors
- Каждая ошибка из счетчиков (spelling_errors, punctuation_errors и т.д.) должна быть описана в examples
  ...
  * Examples: wrong word order, sentence fragments, incomplete sentences
  * IMPORTANT: incomplete sentences (missing subject or predicate) are SYNTAX errors
...
- Выбери подходящую оценку согласно критериям на основе РЕАЛЬНЫХ ошибок`
		...
		const flashRequest: OpenRouterRequest = {
			...
			max_tokens: 4000,
			temperature: 0.2, // Чуть выше для объективной оценки
			top_p: 0.9, // Больший диапазон для проверки ошибок
			top_k: 40, // Стабильность выбора токенов
			metadata: {
		...
		// Минимальная валидация: убираем только пустые примеры ошибок
		if (flashResult.essay_analysis.errors?.examples) {
			flashResult.essay_analysis.errors.examples = flashResult.essay_analysis.errors.examples.filter(error => {
